...
// Validation coverage for SetSand. The live half (writing SandGrid depths, and the Odyssey skip) needs
// a Map and is only exercised by a scenario run; everything a scenario author can get wrong is checked
// here, offline, before a run spends minutes producing frames of bare ground.
[TestFixture]
public class SetSandStepTests
{
    private static bool Validate(out string? error, params (string Key, string Value)[] args)
    {
        ...
        foreach ((string key, string value) in args)
            dict[key] = value;
        return new SetSandStep().TryValidate(dict, out error);
    }

    [Test]
    public void Validate_NoArgs_IsAcceptedAsFullDepthOverTheDefaultRect()
    {
        // `{"type": "SetSand"}` should mean "cover the backdrop", the overwhelmingly common case.
    ...
    {
        SetSandStep step = new SetSandStep();

        // Sand outlives its scenario and changes what every later frame is lit against, so a suite
    ...
    public void Step_IsDiscoveredByTheRegistry()
    {
        ...
        // asserts that is actually held rather than trusting it.
        Assert.That(StepRegistry.KnownTypes, Does.Contain(SetSandStep.StepType));
    }
}
